...
$files=$(Get-ChildItem $args[0] | Measure-Object | ForEach-Object({$_.Count}))
...
$filesize=$("{0:n2}" -f (Get-ChildItem -Recurse $args[0] | Sort-Object (Length -desc | Select-Object -First 1 | Select-Object Fullname, @{Name="Mbytes";Expression={$_.Length / 1Mb}})))
        # Finner gjennomsnittlig filstørrelse i den gitten folderen + subfolders
        #gci->Get-ChildItem (alias)
$averagefileSize=$("{0:n2}" -f ((Get-ChildItem $args[0] -Recurse | Measure-Object -property length -sum).sum /1mb) + "MB")
                # Finner ledig plass på disken ved å differensen mellom
$freespace=$("{0:n2}" -f (( Get-CIMObject Win32_logicaldisk | Where-Object { $_.DeviceID -eq $driveletter } | Measure-Object (-property Freespace -sum).sum /1gb) + "GB"))
            # Finner total plass på disken
$disksize=$("{0:n2}" -f (( Get-CIMObject Win32_logicaldisk | Where-Object { $_.DeviceID -eq $driveletter } | Measure-Object (-property Size -sum).sum /1gb) + "GB"))
